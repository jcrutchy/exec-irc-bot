...
switch ($argv[1])
{
  case "~weather-add":
    ...
    {
      privmsg("usage: \"~weather-add name location\" (visit http://wiki.soylentnews.org/wiki/IRC:exec#Weather_script for more info)");
    }
    break;
  case "~weather":
  ...
  for ($i=0;$i<count($parts);$i++)
  {
    if ((strpos($parts[$i],"/site/")!==False) and (strpos($parts[$i],"[no data]")===False) and (strpos($parts[$i],"[inactive]")===False))
    {
      ...
      if (($j1!==False) and ($j2!==False) and ($j3!==False))
      {
        ...
        $color="10";
        privmsg("weather for ".chr(3).$color.$name.chr(3)." at ".$data_last[0]." (UTC)$agestr");
