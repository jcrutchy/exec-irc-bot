...
$local_list_filename="exec_arthur_list";
...
function refresh_list($update_wiki=True)
{
  global $local_list_filename;
  global $stories_path;
  $file_list=scandir($stories_path);
  $text="<p>TO SUBMIT ONE OF THESE STORIES, TYPE \"~submit-story #\" IN SOYLENTNEWS IRC, WHERE # IS THE ID FROM THE TABLE BELOW</p>".PHP_EOL;
  $text=$text."{| class=\"wikitable\" style=\"width:100%;\"".PHP_EOL;
  $text=$text."|-".PHP_EOL;
  $text=$text."! id !! title !! link".PHP_EOL;
  $local_list=array();
  $id=1;
  for ($i=0;$i<count($file_list);$i++)
  {
    ...
    $text=$text."|-".PHP_EOL;
    $text=$text."| $id || $title || [$url link]".PHP_EOL;
    $local_list[$id]=$filename;
    $id++;
  }
  $text=$text."|}".PHP_EOL;
  $text=$text."<p>autogenerated by [[IRC:exec|exec]] irc bot. don't manually edit this page</p>".PHP_EOL;
  $text=$text."<p>to regenerate from irc, type \"~submit-story list\" in soylentnews irc</p>".PHP_EOL;
  $fn=DATA_PATH.$local_list_filename;
  if (file_put_contents($fn,json_encode($local_list,JSON_PRETTY_PRINT))===False)
  {
    privmsg("error writing local list file");
  }
  if ($update_wiki==False)
  {
    return;
  }
  $title="Storybot";
...
function submit_story($id)
{
  ...
  if (file_exists($fn)==False)
  {
    privmsg("local list file not found");
    return;
  }
  $data=file_get_contents($fn);
  if ($data===False)
  {
    privmsg("error reading local list file");
    return;
  }
  $data=json_decode($data,True);
  if ($data===Null)
  {
    privmsg("error decoding local list file");
    return;
  }
  if (isset($data[$id])==False)
  {
    privmsg("id not found");
    return;
  }
  $story_filename=$stories_path.$data[$id];
